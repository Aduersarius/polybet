...
export function registerCircuitBreakerGauge(callback: (result: { observe: (value: number, attributes?: Record<string, any>) => void }) => void) {
    meter.createObservableGauge('circuit_breaker_state', {
        description: 'Circuit Breaker State (0=CLOSED, 1=OPEN, 2=HALF_OPEN)',
    }).addCallback(callback);
}

export function trackAuthEvent(type: 'login' | '2fa', status: 'success' | 'failure', method?: string) {
    if (type === 'login') {
...
export function trackHedgeEvent(operation: string, status: 'success' | 'failure' | 'partial', duration?: number, extras?: Record<string, any>) {
    const attrs: Record<string, any> = { operation, status, ...extras };
...
export function trackTrade(side: string | Record<string, any>, outcome?: string, amount?: number, type?: string) {
...
export function trackError(errorOrCategory: any, messageOrContext?: any) {
...
export function trackApiLatency(route: string, duration: number, status?: number) {
    const attributes: Record<string, any> = { route };
    if (status) attributes['http.status_code'] = status;
    apiLatencyHistogram.record(duration, attributes);
}

export function trackExternalApi(service: string, operation: string, duration: number, success: boolean = true) {
    externalApiHistogram.record(duration, { service, operation, success: String(success) });
}

export function trackTransaction(type: string, status: string | number, amount?: number | string, token: string = 'USDC') {
