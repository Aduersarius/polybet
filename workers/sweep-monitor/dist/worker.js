...
const getSSLConfig = () => {
    try {
        const caPath = path_1.default.join(process.cwd(), 'certs/db-ca.crt');
        if (fs_1.default.existsSync(caPath)) {
            return {
                ca: fs_1.default.readFileSync(caPath, 'utf8'),
...
const masterNode = ethers_1.ethers.HDNodeWallet.fromPhrase(MNEMONIC, undefined, 'm');
...
async function triggerUserUpdate(userId, type, payload) {
    try {
...
async function sweepDeposit(deposit) {
    ...
    try {
        ...
        const tokenContract = new ethers_1.ethers.Contract(tokenAddress, ERC20_ABI, userWallet);
        ...
        if (maticBalance < requiredMatic) {
            log('INFO', 'Topping up gas', {
                ...
                required: ethers_1.ethers.formatEther(requiredMatic)
            });
        ...
        log('INFO', 'Executing sweep transaction', {
            depositId: deposit.id,
            amount: ethers_1.ethers.formatUnits(balance, 6),
            currency: deposit.currency
        });
        ...
        const rawAmount = parseFloat(ethers_1.ethers.formatUnits(balance, 6));
            ...
            const promises = depositAddresses.flatMap((addr) => {
                ...
                return tokens.map(async (token) => {
                    try {
                        ...
                        const minDeposit = ethers_1.ethers.parseUnits('0.1', 6); // Lowered threshold for polling
...
async function sweepChainDeposit(depositAddress, balance, tokenAddress = USDC_NATIVE_ADDRESS) {
    try {
        ...
        const maxPriorityFeePerGas = feeData.maxPriorityFeePerGas || ethers_1.ethers.parseUnits('35', 'gwei');
        const maxFeePerGas = (feeData.maxFeePerGas || ethers_1.ethers.parseUnits('100', 'gwei')) * 2n; // 2x buffer
        const gasLimit = 100000n;
        const requiredMatic = gasLimit * maxFeePerGas;
        if (maticBalance < requiredMatic) {
            log('INFO', 'Needs gas top-up', {
                address: depositAddress.address,
                balance: ethers_1.ethers.formatEther(maticBalance),
                required: ethers_1.ethers.formatEther(requiredMatic)
            });
            const masterWallet = masterNode.derivePath(`m/44'/60'/0'/0/0`).connect(provider);
