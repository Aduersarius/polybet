...
async function test5MinCandles(tokenId: string, daysAgo: number): Promise<{
  ...
  try {
    ...
    const points: HistoryPoint[] = history
      ...
      .filter((p: any): p is HistoryPoint => p !== null)
      .filter((p: HistoryPoint) => p.timestamp >= startSec && p.timestamp <= endSec)
      .sort((a: HistoryPoint, b: HistoryPoint) => a.timestamp - b.timestamp);
